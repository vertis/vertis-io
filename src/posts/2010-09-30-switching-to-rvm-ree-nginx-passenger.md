...
We're increasing the push toward the release, and I'm now spending precious time looking into the why our Rails Stack is _leaking_ workers. My initial research suggested that these workers could be sent a SIGABRT to get them to throw a stacktrace and exit - this hasn't worked however.
...
**Update:** After changing the stack it appears to have stopped _leaking_ workers - that's a positive outcome to a problem I can't afford the time to look into in more detail. Additionally the before and after performance tests are in:
