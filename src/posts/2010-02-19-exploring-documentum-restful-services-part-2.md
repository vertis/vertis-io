...
```javascript
$.getJSON('/resources/core/repositories/test_repo/folders.json', function (data) {
			...
			function getItems(path) {
				$.getJSON(path, function (data) {
					$.each(data.dataPackage.dataObject, function (i, item) {
						$('#results').append(
							'<a href="#" onclick=\'link("' +
								item.relationshipsUri +
								'");\'>' +
								item.properties.object_name +
								'</a><br/>'
						);
					});
				});
			...
			function getRelationships(path) {
				$.getJSON(path, function (data) {
					$.each(data.relationship, function (i, item) {
						$('#results').append(
							'<a href="#" onclick=\'link("' +
								item.target.relationshipsUri +
								'");\'>' +
								item.target.properties.object_name +
								'</a><br/>'
						);
					});
				});
