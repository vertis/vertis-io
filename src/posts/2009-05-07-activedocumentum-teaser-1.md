...
Having stepped away from trying to get Documentum running on linux, I've been working on bringing the permissions through into our custom Lotus Notes/Documentum client. At the moment when a user imports a document to a given folder it does no permission checking before the fact, just errors nastily when it can't write the document. A short stop by [Prasad's blog](http://askprasad.wordpress.com/documentum/some-useful-dql-tips/) gave me a query to start from.
...
def get_effective_permission(r_object_id, user = nil)
  ...
  folder = DmFolder.find(:first,
    columns: "owner_name, owner_permit, acl_name",
    conditions: { r_object_id: r_object_id }
  )

  current_level = 0
  current_level = folder.owner_permit if folder.owner_name == user

  acls = DmAcl.find(:all,
    columns: "r_accessor_name, r_accessor_permit, r_is_group",
    conditions: "object_name = '#{folder.acl_name}' ENABLE(ROW_BASED)"
  )

  acls.each do |acl|
    if acl.r_accessor_name == user
      current_level = [current_level, acl.r_accessor_permit].max
    elsif acl.r_is_group == '1' && acl.r_accessor_permit
      group = DmGroup.find(:first,
        columns: 'r_object_id, i_all_users_names',
      group = DmGroup.find(:first, {
        :columns => 'r_object_id, i_all_users_names',
        :conditions => {:group_name => acl.r_accessor_name}
      })
      current_level = acl.r_accessor_permit if acl.r_accessor_permit > current_level && group.i_all_users_names.include?(user)
    end
  end
